...
class AXK2Attention(DeepseekV32Attention):
    ...
    def forward(  # type: ignore[override]
        ...
        if mla_slot is None:
            # No KV cache bound yet (memory profiling): skip the cache writes.
            mla_kv_cache = None
            mla_k_scale = None
            indexer_k_cache = None
        else:
            # Keep the fused cache write in the captured region even when
            # attn_metadata is None: breakable PIECEWISE capture runs without
            # attention metadata, and the dummy run fills slot_mapping with -1
            # so the kernel skips at capture and writes real slots on replay.
